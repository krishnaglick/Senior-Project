...
/******/ ([
  ...
  /***/ function(module, exports, __webpack_require__) {
    ...
    window.jsonExport = __webpack_require__(1);
   ...
   function generateCsv(json, callback) {
      ...
      if(_.isArray(json)){
        ...
        json.forEach(function (item) {
              ...
              let itemResult = checkType(item, options.mainPathItem);
              parseResult.push(itemResult);
            });

          //Generate the csv headers based on prop usage over the elements
          //debugger;
          let headers = generateHeaders(parseResult);
          ...
          if(options.includeHeaders){
            ...
            parseResult.forEach(function (result) {
              ...
              let resultRows = [Array(headers.length).join(".").split(".")];
              ...
              result.forEach(function(element){
                ...
                if(!placed){
                  let newRow = Array(headers.length).join(".").split(".");
          ...
          let outputFile = fileRows.join(options.endOfLine || os.EOL || '\n');
      ...
      //Check if the passed json is an Object
        ...
        for(let prop in json){
          let parseResult = checkType(json[prop], prop);
          parseResult.forEach(function (result) {
                  //Type header;value
                  if(options.verticalOutput){
                    let row = [result.item, result.value || options.undefinedString];
                    fileRows.push(row.join(options.rowDelimiter));
                  }else{
   ...
   function checkType(element, item){
    let result = [];
      ...
      if(_.isString(element)){
        let resultString = options.handleString ? options.handleString(element, item) : handleString(element, item);
        result.push({
          item: item,
          value: resultString
        });
      ...
      //Check if element is a Number
        ...
        let resultNumber = options.handleNumber ? options.handleNumber(element, item) : handleNumber(element, item);
        result.push({
          item: item,
          value: resultNumber
        });
      ...
      //Check if element is a Boolean
        ...
        let resultBoolean = options.handleBoolean ? options.handleBoolean(element, item) : handleBoolean(element, item);
        result.push({
          item: item,
          value: resultBoolean
        });
      ...
      //Check if element is a Date
        ...
        let resultDate = options.handleDate ? options.handleDate(element, item) : handleDate(element, item);
        result.push({
          item: item,
          value: resultDate
        });
      ...
      //Check if element is an Array
        ...
        let resultArray = options.handleArray ? options.handleArray(element, item) : handleArray(element, item);
      ...
      //Check if element is a Object
        ...
        let resultObject = options.handleObject ? options.handleObject(element, item) : handleObject(element, item);
   ...
   */
   function handleObject(obj){
    let result = [];
      ...
      for(let prop in obj){
        let propData = obj[prop];
          //Check the propData type
          let resultCheckType = checkType(propData, prop);
          //Append to results
  ...
  /**
   * Handle all Boolean letiables, can be replaced with options.handleBoolean
   *
  ...
  /**
   * Handle all String letiables, can be replaced with options.handleString
   *
   * @param {String} string
  ...
  /**
   * Handle all Number letiables, can be replaced with options.handleNumber
   *
   * @param {Number} number
  ...
  /**
   * Handle all Date letiables, can be replaced with options.handleDate
   *
   * @param {Date} number
   * @returns {string} result
   */
  ...
  let __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;//     Underscore.js 1.8.3
  ...
  (function() {
    ...
    // Save the previous value of the `_` letiable.
    ...
    // Create quick reference letiables for speed access to core prototypes.
    let
    ...
    _.template = function(text, settings, oldSettings) {
      ...
      try {
        let render = new Function(settings.letiable || 'obj', '_', source);
      } catch (e) {
        e.source = source;
        throw e;
      }
      ...
      let argument = settings.letiable || 'obj';
